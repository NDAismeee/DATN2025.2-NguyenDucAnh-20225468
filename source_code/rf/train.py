...
def main() -> None:
    ...
    print(f"[rf train] magic pairs (image-first): {len(pairs)} | image_dir={img_dir} | depth_dir={depth_dir}")
    ...
    rf = RandomForestRegressor(
        ...
        max_depth=None if model_cfg.get("max_depth", None) in (None, "null") else int(model_cfg.get("max_depth")),
    ...
    if val_pairs:
        ...
        for img_path, depth_path, _pid in val_pairs:
            ...
            pred = rf.predict(feats).astype(np.float32, copy=False).reshape(gt.shape)
    ...
    save_dir = Path(str(log_cfg.get("save_dir", "checkpoints_rf"))).expanduser()
    save_dir.mkdir(parents=True, exist_ok=True)
    model_name = str(log_cfg.get("model_name", "random_forest.joblib"))
