...
def main() -> None:
    ...
    print(f"[knn train] magic pairs (image-first): {len(pairs)} | image_dir={img_dir} | depth_dir={depth_dir}")
    ...
    knn = KNeighborsRegressor(
        n_neighbors=int(model_cfg.get("n_neighbors", 5)),
        weights=str(model_cfg.get("weights", "distance")),
        algorithm=str(model_cfg.get("algorithm", "auto")),
        leaf_size=int(model_cfg.get("leaf_size", 30)),
        p=int(model_cfg.get("p", 2)),
        metric=str(model_cfg.get("metric", "minkowski")),
        n_jobs=int(model_cfg.get("n_jobs", -1)),
    )
    model = Pipeline([("scaler", StandardScaler(with_mean=True, with_std=True)), ("knn", knn)])
    model.fit(X, y)
    ...
    if val_pairs:
        ...
        for img_path, depth_path, _pid in val_pairs:
            ...
            pred = model.predict(feats).astype(np.float32, copy=False).reshape(gt.shape)
    ...
    save_dir = Path(str(log_cfg.get("save_dir", "checkpoints_knn"))).expanduser()
    save_dir.mkdir(parents=True, exist_ok=True)
    model_name = str(log_cfg.get("model_name", "knn.joblib"))
