...
_KNN_ROOT = Path(__file__).resolve().parent
_CNN_SRC = _KNN_ROOT.parent / "cnn_src"
if _CNN_SRC.is_dir():
...
def make_vis(
    ...
    *,
    ...
    axes[2].set_title(f"kNN prediction\n{fmt_range(pred_display)} m\n{title_metrics}")
    axes[2].axis("off")
    axes[2].set_aspect("equal")
    add_colorbar(fig, axes[2], im2)

    _panel_na(axes[3], "Raw model μ", "N/A (kNN)")
    _panel_na(axes[4], "Physical prior d_phys", "N/A (kNN)")
    _panel_na(axes[5], "Gate α", "N/A (kNN)")
    _panel_na(axes[6], "Uncertainty var", "N/A (kNN)")
    _panel_na(axes[7], "Zone / distance map", "N/A (kNN)")
...
def main() -> None:
    ...
    load_dotenv(dotenv_path=_KNN_ROOT / ".env", override=True)
    ...
    if rgb_only:
        print(
            "[knn infer] rgb_only=True: no depth; MAE/RMSE are n/a. "
            "Set infer.rgb_only: false and data.depth_dir for GT + metrics."
        )

    ckpt_path = Path(str(infer_cfg.get("checkpoint", "checkpoints_knn_rgb/knn.joblib"))).expanduser()
    out_dir = Path(str(infer_cfg.get("output_dir", "knn_infer_outputs"))).expanduser()
