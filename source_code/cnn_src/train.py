...
def split_pairs_single_or_multi(
    pairs: List[PairType],
    val_ratio: float,
    seed: int,
) -> Tuple[List[PairType], List[PairType]]:
    ...
    return split_pairs(pairs, val_ratio, seed)


def collect_pairs(config: Dict) -> List[PairType]:
    img_dir = Path(config["img_dir"])
    depth_dir = Path(config["depth_dir"])
    pairing = str(config.get("pairing_mode", "prefix")).lower()
    if pairing == "magic":
        return build_pairs_magic(
            img_dir=img_dir,
            depth_dir=depth_dir,
...
def build_datasets(config: Dict, all_pairs: Optional[List[PairType]] = None):
    if all_pairs is None:
        all_pairs = collect_pairs(config)
    ...
    train_pairs, val_pairs = split_pairs_single_or_multi(
        pairs=all_pairs,
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Train only on this patch id (e.g. img_369). Filters matched pairs before split.",
    )
    parser.add_argument(
        "--epochs",
        type=int,
        default=None,
        help="Override train.epochs from YAML.",
    )
    parser.add_argument(
        "--img_glob",
        type=str,
        default=None,
        help="Override data.image_suffix / img_glob (e.g. img_369.tif).",
    )
    parser.add_argument(
        "--image_dir",
        type=str,
        default=None,
        help="Override data.image_dir (otherwise IMAGE_DIR from env / YAML).",
    )
    parser.add_argument(
        "--train_image_dir",
        type=str,
        default="/mnt/disk3/anhnd2468/MagicBathyNet/puck_lagoon/img/aerial_train",
        help="Folder of RGB training images only; pairs GT depth only when a match exists in depth_dir (same as --image_dir, preferred name for folder training).",
    )
    parser.add_argument(
        "--depth_dir",
        type=str,
        default="/mnt/disk3/anhnd2468/MagicBathyNet/puck_lagoon/depth/aerial",
        help="Override data.depth_dir (otherwise DEPTH_DIR from env / YAML).",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
        default=None,
        help="Override train.batch_size from YAML.",
    )
    ...
    if args.img_glob is not None:
        config["img_glob"] = str(args.img_glob)
    if args.train_image_dir is not None:
        config["img_dir"] = str(args.train_image_dir)
    elif args.image_dir is not None:
        config["img_dir"] = str(args.image_dir)
    if args.depth_dir is not None:
        config["depth_dir"] = str(args.depth_dir)
    ...
    if args.patch_stem is not None:
        stem = str(args.patch_stem).strip()
        all_pairs = [p for p in all_pairs if p[2] == stem or p[0].stem == stem]
        if not all_pairs:
            raise ValueError(f"No pair matched --patch_stem={stem!r}")
        print(f"Filtered to {len(all_pairs)} pair(s) for patch_stem={stem!r}")
    if len(all_pairs) < 2:
        print(
            "Single-sample training: train and val loaders both use this patch "
            "(split_pairs requires >=2 samples otherwise)."
        )
        if int(config.get("batch_size", 1)) > 1:
            config["batch_size"] = 1
            print("Set batch_size=1 for single-sample training.")

    train_dataset, val_dataset, stats = build_datasets(config, all_pairs=all_pairs)
