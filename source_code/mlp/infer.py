...
from pixel_mlp_core import pick_torch_device, predict_from_bundle
...
def make_vis(
    ...
    *,
    ...
    if rgb_only:
        ...
        im1 = axes[1].imshow(gt_display, cmap=DEPTH_CMAP, vmin=DISPLAY_DEPTH_MIN, vmax=DISPLAY_DEPTH_MAX, interpolation=DEPTH_INTERPOLATION)
        axes[1].set_title(f"Ground truth\n{fmt_range(gt_display)} m")
        axes[1].axis("off")
        axes[1].set_aspect("equal")
        add_colorbar(fig, axes[1], im1)

    im2 = axes[2].imshow(pred_display, cmap=DEPTH_CMAP, vmin=DISPLAY_DEPTH_MIN, vmax=DISPLAY_DEPTH_MAX, interpolation=DEPTH_INTERPOLATION)
    title_metrics = "n/a" if rgb_only else f"MAE={mae:.3f} RMSE={rmse:.3f}"
    axes[2].set_title(f"MLP prediction\n{fmt_range(pred_display)} m\n{title_metrics}")
    axes[2].axis("off")
    axes[2].set_aspect("equal")
    add_colorbar(fig, axes[2], im2)

    _panel_na(axes[3], "Raw model μ", "N/A (MLP)")
    _panel_na(axes[4], "Physical prior d_phys", "N/A (MLP)")
    _panel_na(axes[5], "Gate α", "N/A (MLP)")
    _panel_na(axes[6], "Uncertainty var", "N/A (MLP)")
    _panel_na(axes[7], "Zone / distance map", "N/A (MLP)")
...
def main() -> None:
    ...
    if rgb_only:
        print(
            "[mlp infer] rgb_only=True: no depth; MAE/RMSE are n/a. "
            "Set infer.rgb_only: false and data.depth_dir for GT + metrics."
        )

    ckpt_path = Path(str(infer_cfg.get("checkpoint", "checkpoints_mlp_rgb/mlp.joblib"))).expanduser()
    out_dir = Path(str(infer_cfg.get("output_dir", "mlp_infer_outputs"))).expanduser()
    out_dir.mkdir(parents=True, exist_ok=True)

    bundle = load(ckpt_path)
    infer_dev = str(infer_cfg.get("device") or train_cfg.get("device", "cpu"))
    infer_device = pick_torch_device(infer_dev, int(train_cfg.get("gpu_id", 0)))
    ...
    if rgb_only:
        ...
        for img_path in paths:
            ...
            rgb_preview = chw_rgb_preview(img[:3] / float(reflectance_scale))
            rgb = _rgb_hw3(img, reflectance_scale=reflectance_scale)
            feats = _pixel_features(rgb, include_xy=include_xy)
            pred_pos = predict_from_bundle(bundle, feats, chunk=200000, device=infer_device).reshape(rgb.shape[:2])
            ...
            make_vis(vis_path, rgb_preview, z, pred_pos, z.astype(np.uint8), float("nan"), float("nan"), rgb_only=True)
            rows.append(
                {
                    "id": pid,
        ...
        for img_path, depth_path, pid in pairs:
            ...
            feats = _pixel_features(rgb, include_xy=include_xy)
            pred_pos = predict_from_bundle(bundle, feats, chunk=200000, device=infer_device).reshape(gt_pos.shape)
            ...
            vis_path = out_dir / f"{pid}_vis.png"
            make_vis(vis_path, rgb_preview, gt_pos, pred_pos, valid, mae, rmse, rgb_only=False)
